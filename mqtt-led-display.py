...
def on_message(client, userdara, msg):
    if msg.topic == MQTT_TOPIC:
        ...
        last_temperature_received = str(payload["actual_temparature"])
        # display height is 7, save the bottom row for the graph
        scrollphathd.clear_rect(0,0,None,6) # None defaults to display size
        scrollphathd.write_string(last_temperature_received, brightness=0.5, font=font3x4)
        scrollphathd.show()
        ...
        scrollphathd.fill(brightness, 17 - (last_load_received/8.0)/17, 6)
        scrollphathd.show()
